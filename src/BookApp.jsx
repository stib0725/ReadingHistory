  ...
  useEffect(() => {
    const loadScripts = async () => {
      try {
        ...
        // 2. 環境変数の取得（修正箇所）
        // Viteの場合: import.meta.env.VITE_SUPABASE_URL
        // Create React Appの場合: process.env.REACT_APP_SUPABASE_URL
        ...
        if (supabaseUrl && supabaseAnonKey && window.supabase) {
          ...
          setDebugInfo('接続情報（URL/Key）が見つかりません。.envを確認してください。');
        }
  ...
  return (
    <div style={styles.container}>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <button type="submit" style={{ width: '100%', padding: '12px', background: '#007bff', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>{formData.id ? "更新する" : "保存する"}</button>
      </form>
      {/* ... (残りのUI部分は変更なし) ... */}
      <div style={{ marginBottom: '15px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <div style={styles.tabScroll}>{['すべて', '★', ...statuses].map(s => (<button key={s} onClick={() => setStatusFilter(s)} style={styles.statusTab(statusFilter === s, s === '★' ? '#f1c40f' : '#007bff')}>{s}</button>))}</div>
      ...
      <div>
        {filteredBooks.map(book => (
          <div key={book.id} style={styles.card} onClick={() => { setFormData({...book, finish_date: book.finish_date || ''}); window.scrollTo({top:0, behavior:'smooth'}); }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{display: 'flex', alignItems: 'center', marginBottom: '2px'}}><span style={styles.badge(book.status)}>{book.status}</span><span style={styles.categoryBadge(book.category)}>{book.category}</span></div>
              <div style={{ fontWeight: 'bold', fontSize: '14px' }}>{book.title}</div>
              <div style={{ fontSize: '12px', color: '#666' }}>{book.author}</div>
              {book.finish_date && <div style={{ fontSize: '11px', color: '#2c3e50', marginTop: '6px', backgroundColor: '#eef2f7', padding: '2px 6px', borderRadius: '4px', display: 'inline-block' }}>🏁 <span style={{fontWeight: 'bold'}}>{book.finish_date}</span> 読破！</div>}
            </div>
          </div>
        ))}
      </div>
